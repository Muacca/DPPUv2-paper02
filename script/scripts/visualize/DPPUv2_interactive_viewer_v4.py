...
#   - dppu package on sys.path  (add _DPPUv2_Phase2/ two levels up)
#
# Features:
...
#   - α_W (Weyl coupling): adds α·C² term to Lagrangian   ← NEW
#   - Weyl status indicator: dynamic C²=0 / C²≠0 feedback  ← NEW
...
#
# v4 Engine Changes vs v3:
#   - Uses dppu package (modular, object-oriented architecture)
#   - ε and α_W remain symbolic in engine → ONE engine run per
#     (topology, mode, ny_variant); evaluated numerically per scan point
#   - Cache key: (topology, mode, ny_variant) only  [not (ε, α) variants]
#   - Lagrangian: L = R/(2κ²) + θ_NY·N + α_W·C²
#   - T³ anisotropy (v3 R2/R3 feature) removed: v4 T³ is isotropic by design
#     (structure constants all zero; R2=R3=r=R1 in engine)
#
# UI Layout:
...
try:
    ...
    print("✓ dppu v4 engine modules imported successfully.")
except ImportError as e:
    print(f"✗ Error importing dppu: {e}")
    print("  Ensure _DPPUv2_Phase2/ is on sys.path before importing this module.")
    raise
...
def get_potential_function(topology: str, mode: str, ny_variant: str,
    ...
    v4 design: ε (squashing) and α_W (Weyl coupling) are kept as symbolic
    parameters. The returned callable accepts them as numerical arguments,
    so a single engine run covers all (ε, α) values.
    ...
    if status_callback:
        status_callback(
            ...
            f" (30–120 s, ε/α_W will be numerical after this)"
        )
    ...
    # Run engine with output suppressed (NullLogger used by default)
    ...
    # Detect which torsion parameters are actual SymPy symbols
    # (AX mode: V = S.Zero; VT mode: eta = S.Zero)
    def _is_sym(x):
        return hasattr(x, 'is_Symbol') and x.is_Symbol

    V_is_symbol   = _is_sym(V_sym)
    eta_is_symbol = _is_sym(eta_sym)
    # epsilon and alpha are always symbols in v4 engines

    # Build lambdify argument list (only proper Symbol instances)
    lambda_args = [r_sym]
    if V_is_symbol:
        lambda_args.append(V_sym)
    if eta_is_symbol:
        lambda_args.append(eta_sym)
    lambda_args.extend([theta_sym, L_sym, kappa_sym, epsilon_sym, alpha_sym])

    raw_func = lambdify(lambda_args, sym_expr, modules='numpy')

    # Unified wrapper: f(r, V_param, eta_param, theta_NY, epsilon, alpha)
    # V_param / eta_param are ignored in modes where they are not symbols.
    if mode == 'MX':
        def wrapped_func(r, V_param, eta_param, theta_NY, epsilon=0.0, alpha=0.0):
            return raw_func(r, V_param, eta_param, theta_NY,
                            L_SCALE, KAPPA, epsilon, alpha)
    elif mode == 'AX':
        def wrapped_func(r, V_param, eta_param, theta_NY, epsilon=0.0, alpha=0.0):
            # V_param ignored (V = 0 fixed in AX mode)
            return raw_func(r, eta_param, theta_NY,
                            L_SCALE, KAPPA, epsilon, alpha)
    elif mode == 'VT':
        def wrapped_func(r, V_param, eta_param, theta_NY, epsilon=0.0, alpha=0.0):
            # eta_param ignored (η = 0 fixed in VT mode)
            return raw_func(r, V_param, theta_NY,
                            L_SCALE, KAPPA, epsilon, alpha)
    else:
...
def get_weyl_c2_status(topology: str, epsilon: float):
    ...
    Physical rules:
      - T³×S¹:        C² ≡ 0  (flat torus, always conformally flat)
      - S³×S¹  ε=0:  C² = 0  (round sphere, conformally flat)
      - Nil³×S¹ ε=0: C² = 0  (undeformed Heisenberg, conformally flat)
      - S³/Nil³ ε≠0: C² ≠ 0  (squashed → Weyl curvature)
    ...
    if abs(epsilon) < 1e-10:
        msg = (
            "ε = 0 (isotropic): C² = 0 — "
            "<b>α_W has NO effect</b> (conformally flat)"
        )
        return True, msg, "#757575"

    msg = (
        f"ε = {epsilon:.3f} (squashed): C² ≠ 0 — "
        "<b>α_W IS active</b>"
    )
